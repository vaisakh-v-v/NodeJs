...
export async function handleCreateUser(req: Request, res: Response) {
  ...
  if (!requestBody.role) {
    throw new HTTPError("role not found", 401);
  }

  const roles = ["adminstrator", "customer", "agent"];

  if (!roles.includes(requestBody.role)) {
    throw new HTTPError("invalid role", 401);
  }
...
export async function handleUserLogin(req: Request, res: Response) {
  ...
  const user: Omit<User, "name" | "user_id" | "role"> = {
    email: requestBody.email,
    hashed_password: createHash("sha256")
      .update(requestBody.password)
      .digest("base64"),
